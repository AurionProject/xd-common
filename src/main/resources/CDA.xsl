                        ...
                        <td colspan="3">
                           <xsl:choose>
                              <xsl:when test="n1:location/n1:healthCareFacility/n1:location/n1:name">
								 ...
								 <br/>
								 <!-- Changes Start: Added by vshah on 04/23/2014 on request from Becky that we need to display full address -->
   ...
   <xsl:template name="show-language">
      ...
      <xsl:choose>
         <xsl:when test="@code   = 'nob'">
            <xsl:text>Bokmal, Norwegian</xsl:text>
         </xsl:when>
      </xsl:choose>
      <xsl:choose>
